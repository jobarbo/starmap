...
function INIT() {
	...
	for (let i = 0; i < particleNum; i++) {
		...
		movers.push(new Mover(x, y, initHue, scl1 / MULTIPLIER, scl2 / MULTIPLIER, amp1 * MULTIPLIER, amp2 * MULTIPLIER, xMin, xMax, yMin, yMax, xRandDivider, yRandDivider, seed));
	}
...
class Mover {
	...
	move(frameCount) {
		...
		this.lineWeightMax = map(frameCount, 150, MAX_FRAMES - 150, 40, 1, true);
		this.skipperMax = map(frameCount, 150, MAX_FRAMES - 150, 20, 0.1, true);
		...
		this.lineWeight = map(totalSpeed, 0, 30002 * MULTIPLIER, 0, this.lineWeightMax, true) * MULTIPLIER; //* Seem to work now
		//this.lineWeight = 1 * MULTIPLIER;
